...
public:
	...
	void tick_extclock(int which) {
		class channel& c = channels[which];
		c.gra = c.tcnt;
		c.tsr |= 1;
		if ((c.tcr & 0x60) == 0x20) c.tcnt = 0;
		if (c.tier & 1) state->interrupt(24 + which * 4);
	}
	void tick()
	{
		for (int i = 0; i < 5; i++)
...
private:	// 0x9f -> 0x60
	...
	public:
		...
		uint8 tcr {128}, tsr {0xf8}, tier {0xf8}, tior {0x88};
	};
	channel channels[5];
};
